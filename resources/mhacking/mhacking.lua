	...
	while true do
		...
		if showHelp then
			if helpTimer > GetGameTimer() then
				showHelpText("Navegue com ~y~W,A,S,D~s~ e confirme com ~y~SPACE~s~ para o bloco da direita.")
			elseif helpTimer > GetGameTimer()-helpCycle then
				showHelpText("Use as ~y~Setass~s~ e a tecla ~y~ENTER~s~ para o código da direita")
			else
