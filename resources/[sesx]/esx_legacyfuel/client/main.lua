		...
		if not InBlacklistedVehicle then
			...
			elseif NearVehicleWithJerryCan and not nearPump and Config.EnableJerryCans then
				...
				if IsFuelingWithJerryCan then
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Pressione ~g~G ~w~para cancelar o abastecimento. Nível atual de Combustível: " .. fuel .. "% - Galão: " .. jerrycan)
					...
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Pressione ~g~G ~w~para abastecer o seu veículo com o galão.")
...
function DisplayHud()
	...
	if Config.ShouldDisplayHud and IsPedInAnyVehicle(playerPed, false) and GetSeatPedIsIn(playerPed) == -1 then
		...
		DrawAdvancedText(0.148 - x, 0.7765 - y, 0.005, 0.0028, 0.4, "mp/h              km/h              Gasolina", 255, 255, 255, 255, 6, 1)
	end
end
