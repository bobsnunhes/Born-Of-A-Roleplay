...
function JailLogin()
	...
	ESX.ShowNotification("A última vez que você caiu no sono você estava na prisão, então você está aqui.")
...
function UnJail()
	...
	ESX.ShowNotification("Agora você está livre, nós deixamos você no registro, então vá com calma.")
end
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			ESX.ShowNotification("Você tem " .. jailTime .. " minutos restantes na prisão!")
		...
		while jailTime > 0 do
			...
			for posId, v in pairs(Packages) do
				...
				if DistanceCheck <= 10.0 then
					...
					local PackageText = "pacote"

					if not v["state"] then
						PackageText = "Já embalado"
					end
					...
					if DistanceCheck <= 1.5 then
						...
						if IsControlJustPressed(0, 38) then
							...
							if v["state"] then
								...
								ESX.ShowNotification("Você já pegou este pacote.")
							end
		...
		while true do
			...
			for p, v in pairs(Config.Teleports) do
				...
				if DistanceCheck <= 7.5 then
					...
					ESX.Game.Utils.DrawText3D(v, "[E] Abra a porta", 1)
...
function PackPackage(packageId)
	...
	while Packaging do
		...
		if PackPercent >= 100 then
			...
			ESX.Game.Utils.DrawText3D(Package, "pacote... " .. math.ceil(tonumber(PackPercent)) .. "%", 0.4)
		end
...
function OpenJailMenu()
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'jail_prison_menu',
		{
			...
			align    = 'top-left',
			elements = {
				{ label = "Prender a pessoa mais próxima", value = "jail_closest_player" },
				{ label = "Soltar pessoal da prisão", value = "unjail_player" }
			}
		}, 
	function(data, menu)
		...
		if action == "jail_closest_player" then
			...
			ESX.UI.Menu.Open(
          		...
          		{
            		title = "Período de prisão (minutos)"
          		},
          	function(data2, menu2)
            	...
            	if jailTime == nil then
              		ESX.ShowNotification("O tempo deve estar em minutos.")
            	else
              		menu2.close()
              		...
              		if closestPlayer == -1 or closestDistance > 3.0 then
                		ESX.ShowNotification("Ninguém por perto.")
					else
						ESX.UI.Menu.Open(
						...
						function(data3, menu3)
						  	...
						  	if reason == nil then
								ESX.ShowNotification("Você precisa escrever algo aqui.")
						  	else
								menu3.close()
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
				...
				if #playerArray == 0 then
					ESX.ShowNotification("Seu celular está vazio!")
					return
				end

				for i = 1, #playerArray, 1 do
					table.insert(elements, {label = "prisioneiro: " .. playerArray[i].name .. " | prisão: " .. playerArray[i].jailTime .. " minuter", value = playerArray[i].identifier })
				end
