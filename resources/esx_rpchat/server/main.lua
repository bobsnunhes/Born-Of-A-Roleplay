...
RegisterCommand('anontweet', function(source, args, rawCommand)
    ...
    fal = name.firstname .. " " .. name.lastname
    TriggerClientEvent('chat:addMessage', -1, {
        template = '<div style="padding: 0.5vw; margin: 0.5vw; background-color: rgba(28, 160, 242, 0.6); border-radius: 3px;"><i class="fab fa-twitter"></i> @Anônimo:<br> {1}</div>',
        args = { fal, msg }
    })
end, false)
...
RegisterCommand('ad', function(source, args, rawCommand)
    ...
    fal = name.firstname .. " " .. name.lastname
    TriggerClientEvent('chat:addMessage', -1, {
        template = '<div style="padding: 0.5vw; margin: 0.5vw; background-color: rgba(214, 168, 0, 1); border-radius: 3px;"><i class="fas fa-ad"></i> Anúncio:<br> {1}<br></div>',
        args = { fal, msg }
    })
end, false)
