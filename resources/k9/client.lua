...
local Keys = {
    ...
    ["K"] = 311, ["L"] = 182, ["LEFTSHIFT"] = 21, ["Z"] = 20, ["X"] = 73, ["C"] = 26, ["V"] = 0, ["B"] = 29, ["N"] = 249, ["M"] = 244, [","] = 82, ["."] = 81,
...
--[[ Main Event Handlers ]]--
    ...
    AddEventHandler("K9:OpenMenu", function(pedRestriction, pedList)
        if pedRestriction then
            if CheckPedRestriction(GetLocalPed(), pedList) then
                ...
                Notification(tostring("~r~Você não tem o uniforme correto para utilizar o K9."))
            end
        else
            EnableMenu()
        end
    ...
    AddEventHandler("K9:IdentifierRestricted", function()
        Notification(tostring("~r~Você não está na whitelist do K9."))
    end)
    ...
    AddEventHandler("K9:SearchVehicle", function(items, openDoors)
        ...
        if vehicle ~= 0 and not searching then
            ...
            Notification(tostring(k9_name .. " começou a procurar..."))
...
--[[ Threads ]]
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            -- Trigger Opens Menu
            if IsControlPressed(1, Keys['LEFTALT']) and IsControlJustPressed(1, Keys['X']) then
